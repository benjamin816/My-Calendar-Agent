...
export default function ChronosApp() {
  ...
  const deleteTask = async (task: CalendarTask) => {
    ...
    setIsProcessing(true);
    try {
      await calendarService.deleteTask(task.id, session.accessToken as string);
      await refreshData();
    } catch (e) { console.error(e); }
    finally { setIsProcessing(false); }
  };
                ...
                {activeTab === 'calendar' ? (
                  <>
                    <div className="lg:hidden flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-white overscroll-contain">
                      ...
                      {getEventsForDay(currentDate).length === 0 && <EmptyDayView label="No Events" />}
                    </div>
                    {displayDays.map(day => (
                      <div key={day.toISOString()} className="hidden lg:block border-l first:border-l-0 border-slate-100 p-4 space-y-4 overflow-y-auto custom-scrollbar hover:bg-slate-50/50 transition-colors overscroll-contain">
                        ...
                        {getEventsForDay(day).length === 0 && <EmptyDayView label="No Events" />}
                      </div>
                    ))}
                  </>
                ) : (
                  <>
                    <div className="lg:hidden flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-white overscroll-contain">
                      {getTasksForDay(currentDate).map(t => (
                        <TaskCard key={t.id} task={t} onToggle={() => toggleTask(t)} onDelete={() => deleteTask(t)} />
                      ))}
                      {getTasksForDay(currentDate).length === 0 && <EmptyDayView label="No Tasks" />}
                    </div>
                    {displayDays.map(day => (
                      <div key={day.toISOString()} className="hidden lg:block border-l first:border-l-0 border-slate-100 p-4 space-y-4 overflow-y-auto custom-scrollbar hover:bg-slate-50/50 transition-colors overscroll-contain">
                        ...
                        {getTasksForDay(day).length === 0 && <EmptyDayView label="No Tasks" />}
                      </div>
                    ))}
                  </>
                )}
...
function TaskCard({ task, onToggle, onDelete }: { task: CalendarTask, onToggle: () => void, onDelete: () => void }) {
  return (
    <div className={cn(
      "bg-white border p-3 lg:p-4 rounded-2xl shadow-sm hover:shadow-md transition-all group flex items-start gap-3",
      ...
      <button onClick={onToggle} className={cn(
        ...
        task.completed ? "bg-green-500 border-green-500" : "border-slate-200 hover:border-blue-500"
      )}>
        {task.completed && <CheckIcon className="w-4 h-4 text-white stroke-[3]" />}
      </button>
      <div className="flex-1 min-w-0">
        <p className={cn("text-sm font-bold text-slate-900 leading-snug truncate", task.completed && "line-through text-slate-400")}>{task.title}</p>
        {task.notes && <p className="text-[10px] text-slate-400 mt-1 line-clamp-1">{task.notes}</p>}
      </div>
      <button onClick={onDelete} className="text-slate-200 hover:text-red-500 transition-colors p-1 opacity-0 group-hover:opacity-100">
...
function EmptyDayView({ label = "Clear Schedule" }: { label?: string }) {
  return (
    <div className="h-full py-10 flex flex-col items-center justify-center opacity-10">
      ...
      <p className="text-[10px] font-bold mt-2 uppercase tracking-widest">{label}</p>
    </div>
  );
}
    ...
    <div className="flex-1 flex flex-col overflow-hidden h-full">
      ...
      <div className="p-4 lg:p-8 border-t border-slate-100 bg-white flex-shrink-0">
        <div className="flex items-center gap-2 mb-2 lg:hidden">
          <button onClick={onReset} className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1.5 px-2 py-1 hover:text-blue-600">
            <ArrowUturnLeftIcon className="w-3 h-3"/> Reset Chat
          </button>
        </div>
